...
function generate (tasks, callback) {
  ...
  function each (done, index) {
    setup(tasks[index], done);
  }
}


function save (target, tasks, callback) {
  install('bud', { saveDev: true }, function (error) {
    if (error) {
      fail('Can not install bud. Please run `npm install bud --save-dev`');
    };
  });
...
function setup (task, callback) {
  console.log('');
  info('Setting up ' + task.module);

  install(task.module, { saveDev: true }, function (error) {
    if (error) {
    ...
    task.plugin.params && task.plugin.params.forEach(function (p, index) {
      options[p.key || ('p' + index)] = option(p.name || p, p.description || p.desc);
    });
